...
const T = {
  en: {
    dir: "ltr", nav_title: "Prayer Times", nav_cta: "Subscribe",
    hero_pre: "🕌 Jummah Reminder Service",
    hero_title: "أوقات الصلاة", hero_subtitle: "Prayer Times",
    ...
    prayer_section: "Today's Prayers", prayer_city: "Prayer times for", change_city: "Change",
    fajr: "Fajr", dhuhr: "Dhuhr", asr: "Asr", maghrib: "Maghrib", isha: "Isha",
    subscribe_pre: "Never Miss Jummah", subscribe_title: "Get Weekly Reminders",
    ...
    city_label: "Your city", city_placeholder: "e.g. Casablanca",
    lang_label: "Language preference", subscribe_btn: "Subscribe Free", subscribing: "Subscribing...",
    ...
    footer_tagline: "Built with ❤️ for the Muslim community.", footer_rights: "All rights reserved.",
    current: "Current", next: "Next", loading_prayers: "Updating...",
    city_form_btn: "Get Prayer Times",
  },
  fr: {
    dir: "ltr", nav_title: "Horaires de Prière", nav_cta: "S'abonner",
    hero_pre: "🕌 Service de Rappel Jummah",
    hero_title: "أوقات الصلاة", hero_subtitle: "Horaires de Prière",
    ...
    prayer_section: "Prières du Jour", prayer_city: "Horaires pour", change_city: "Changer",
    fajr: "Fajr", dhuhr: "Dhohr", asr: "Asr", maghrib: "Maghrib", isha: "Ichaa",
    subscribe_pre: "Ne Ratez Plus le Jummah", subscribe_title: "Rappels Hebdomadaires",
    ...
    city_label: "Votre ville", city_placeholder: "ex: Casablanca",
    lang_label: "Langue préférée", subscribe_btn: "S'abonner Gratuitement", subscribing: "Inscription...",
    ...
    footer_tagline: "Fait avec ❤️ pour la communauté musulmane.", footer_rights: "Tous droits réservés.",
    current: "Actuelle", next: "Prochaine", loading_prayers: "Mise à jour...",
    city_form_btn: "Voir les Horaires",
  },
  ar: {
    dir: "rtl", nav_title: "أوقات الصلاة", nav_cta: "اشترك",
    hero_pre: "🕌 خدمة تذكير الجمعة",
    hero_title: "أوقات الصلاة", hero_subtitle: "مواقيت الصلاة",
    ...
    prayer_section: "صلوات اليوم", prayer_city: "مواقيت", change_city: "تغيير",
    fajr: "الفجر", dhuhr: "الظهر", asr: "العصر", maghrib: "المغرب", isha: "العشاء",
    subscribe_pre: "لا تفوّت صلاة الجمعة", subscribe_title: "تذكيرات أسبوعية",
    subscribe_desc: "تلقّ موعد صلاة الجمعة لمدينتك كل جمعة صباحاً.",
    email_label: "البريد الإلكتروني", email_placeholder: "you@example.com",
    city_label: "مدينتك", city_placeholder: "مثال: الدار البيضاء",
    lang_label: "اللغة المفضلة", subscribe_btn: "اشترك مجاناً", subscribing: "جارٍ الاشتراك...",
    ...
    footer_tagline: "بُني بـ ❤️ للمجتمع المسلم.", footer_rights: "جميع الحقوق محفوظة.",
    current: "الحالية", next: "القادمة", loading_prayers: "جارٍ التحديث...",
...
function getCurrentPrayer(timings) {
  ...
  for (let i = 0; i < times.length; i++) {
    if (nowMins >= times[i].mins) { current = times[i]; next = times[(i + 1) % times.length]; }
  }
...
export default function PrayerClient({ initialCity, initialCountry, initialTimings }) {
  ...
  const [jummah, setJummah] = useState(getJummah);
  const [city, setCity] = useState(initialCity);
  const [country, setCountry] = useState(initialCountry);
  const [prayers, setPrayers] = useState(initialTimings);
  const [prayerStatus, setPrayerStatus] = useState(() => initialTimings ? getCurrentPrayer(initialTimings) : null);
  ...
  useEffect(() => {
    const id = setInterval(() => setJummah(getJummah()), 1000);
  ...
  const fetchCity = useCallback(async (c, co, silent = false) => {
    const cached = getCached(c, co);
    if (cached) { setPrayers(cached); setPrayerStatus(getCurrentPrayer(cached)); return; }
    if (!silent) setLoadingCity(true);
    try {
    ...
    if (!silent) setLoadingCity(false);
  }, []);

  // Background: detect real city via IP, silently upgrade if different from default
  useEffect(() => {
    ...
    fetch("https://ip-api.com/json/?fields=city,country")
      ...
      .catch(() => {});
  }, [fetchCity, initialCity, initialCountry, initialTimings]);

  const handleCityChange = (e) => {
    e.preventDefault();
    if (!cityInput.trim()) return;
    const newCity = cityInput.trim();
    setCity(newCity); setShowCityForm(false); setCityInput("");
    fetchCity(newCity, "");
  };
  ...
  const handleSubscribe = async (e) => {
    ...
    try {
      ...
      const data = await r.json();
      if (!r.ok) setSubError(r.status === 409 ? t.already_sub : (data.error || "Something went wrong."));
      else setSubSuccess(true);
    } catch { setSubError("Network error. Please try again."); }
    setSubLoading(false);
  };

  const countdownParts = [
    { label: t.days, val: jummah.days }, { label: t.hours, val: jummah.hours },
    { label: t.mins, val: jummah.mins }, { label: t.secs, val: jummah.secs },
  ];
  ...
  return (
    <div className="min-h-screen bg-bg-base pattern-bg" dir={t.dir}>
      ...
      <main className="relative z-10">
        ...
        <section className="min-h-screen flex flex-col items-center justify-center text-center px-4 pt-16">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="max-w-3xl mx-auto">
            ...
            <h1 className="font-arabic text-6xl sm:text-8xl font-bold gold-shimmer leading-tight mb-2">{t.hero_title}</h1>
                  ...
                  <div key={label} className="bg-bg-base/60 rounded-2xl p-3 sm:p-4">
                    <AnimatePresence mode="wait">
                      <motion.div key={val} initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }}
                        transition={{ duration: 0.15 }} className="text-2xl sm:text-4xl font-bold text-white font-mono tabular-nums">
                ...
                {PRAYERS.map((p, i) => {
                  ...
                  return (
                    <motion.div key={p.key} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
                      ...
                      <p className="font-arabic text-sm sm:text-base mb-0.5" style={{ color: isCurrent ? p.color : "rgba(255,255,255,0.7)" }}>{t[p.tKey]}</p>
                      ...
                      {isCurrent && <span className="mt-2 inline-block text-[10px] px-2 py-0.5 rounded-full" style={{ background: `${p.color}20`, color: p.color }}>{t.current}</span>}
                      {isNext && !isCurrent && <span className="mt-2 inline-block text-[10px] px-2 py-0.5 rounded-full bg-emerald/15 text-emerald">{t.next}</span>}
                    </motion.div>
                  );
                })}
              </div>
                ...
                {subSuccess ? (
                  ...
                  <motion.form key="form" onSubmit={handleSubscribe} className="space-y-4">
                    <div>
                      ...
                      <input type="email" value={subEmail} onChange={e => setSubEmail(e.target.value)} required placeholder={t.email_placeholder}
                    ...
                    <div>
                      ...
                      <input type="text" value={subCity} onChange={e => setSubCity(e.target.value)} required placeholder={t.city_placeholder}
                        className="w-full bg-bg-base/60 border border-white/8 rounded-xl px-4 py-3 text-white placeholder-white/25 focus:outline-none focus:border-gold/40 transition-all text-sm" />
                    </div>
                    <div>
                    ...
                    <button type="submit" disabled={subLoading}
                      ...
                      }`}>{subLoading ? t.subscribing : t.subscribe_btn}</button>
      ...
      <footer className="border-t border-white/5 py-8 px-4 text-center">
        ...
        <div className="mt-3"><a href="/admin/login" className="text-white/15 text-xs hover:text-white/30 transition-colors">Admin</a></div>
      </footer>
    </div>
  );
}
